...
;pipe
(define-struct pipe-struct (x gap-y) #:transparent)
;variabile
(define-struct variables-struct (gravity momentum scroll-speed) #:transparent)
;state
(define-struct state (bird pipes score) #:transparent)

;definesc structurile statice
(define bird (bird-struct bird-x  bird-initial-y 0))
(define variables (variables-struct initial-gravity  initial-momentum initial-scroll-speed))

;starea initiala
(define (get-initial-state)
       (define pipe (pipe-struct scene-width (+ added-number (random random-threshold))))
       (define initial-state (state bird (list pipe) 0))
  initial-state)
...
(define (next-state-bird bird gravity)
  (match-let ([(bird-struct x y v-y) bird])
    (struct-copy bird-struct bird  [y (+ y v-y)] [v-y (+ v-y gravity)])))
;TODO 4
...
(define (next-state-bird-onspace bird momentum)
  (match-let ([(bird-struct x y v-y) bird])
    (struct-copy bird-struct bird [v-y (- momentum)])))
...
(define (change current-state pressed-key)
   (match-let ([(bird-struct x y v-y) (get-bird current-state)])
    (if (key=? pressed-key " ") (make-state (next-state-bird-onspace (get-bird current-state) initial-momentum)
...
(define (move-pipes pipes scroll-speed)
  (map (lambda(p)
        (match-let ([(pipe-struct x gap-y) p])
          (struct-copy pipe-struct p [x (- (pipe-struct-x p) scroll-speed)]))) pipes))
;TODO 12
...
;
; Hint: cunoaștem lățimea unui pi pe, pipe-width
(define (clean-pipes pipes)
  (filter (lambda (pipe)
          (<  0 (+ (get-pipe-x pipe) pipe-width))) pipes))
...
(define (add-more-pipes pipes)
 (if (>= (length pipes) no-pipes)
     pipes
     (let [(lx (get-pipe-x (car pipes)))]
       (append (list (pipe-struct (+ pipe-width (+ pipe-gap lx)) (+ added-number (random random-threshold)))) pipes))))
...
(define (check-ground-collision bird)
 (cond ((>= (+ (bird-struct-y bird) bird-height) ground-y) #t)
       (else #f)))
; invalid-state?
...
(define (invalid-state? state)
  (cond [(equal? (check-ground-collision (get-bird state)) #t)  #t]
    (else (check-pipe-collisions (get-bird state) (get-pipes state)))
     ))
;TODO 21
...
(define (check-pipe-collisions bird pipes)
   (cond [(null? pipes)  #f]
      [else (match-letrec ([(pipe-struct x gap-y) (first pipes)]
             (A (posn x  (- gap-y pipe-height)))
             (B (posn (+ pipe-width x) gap-y)) 
             (C (posn x (+ pipe-self-gap gap-y))) 
             (D (posn (+ x pipe-width) (+ pipe-self-gap gap-y pipe-height)))
             (E (posn bird-x (get-bird-y bird)))
             (F (posn (+ bird-width bird-x) (+ (get-bird-y bird) bird-height))))
             (cond ((equal? (or (check-collision-rectangles A B E F) (check-collision-rectangles C D E F)) #f) (check-pipe-collisions bird (cdr pipes)))
             (else #t)
       ))]))
...
(define (next-state state)
   (make-state (next-state-bird (get-bird state) initial-gravity)
               (next-state-pipes (get-pipes state) initial-scroll-speed) (+ (get-score state) 0.1)))
...
(define (place-pipe a)
  (rectangle pipe-width (get-pipe-gap-y a) "solid" "green"))

(define (place-pipe2 a)
  (rectangle  pipe-width (- scene-height (get-pipe-gap-y a) pipe-self-gap) "solid" "green"))

; Folosind `place-image/place-images` va poziționa pipe-urile pe scenă.
(define (place-pipes pipes scene)
  (cond [(null? pipes) scene]
  [else (place-image (place-pipe (first pipes)) (+ (get-pipe-x (first pipes)) (quotient pipe-width 2))
                                         (quotient (get-pipe-gap-y (first pipes)) 2) (place-image (place-pipe2 (first pipes))
                                             (+ (get-pipe-x (first pipes)) (quotient pipe-width 2)) (+ (get-pipe-gap-y (first pipes)) pipe-self-gap
                                                    (quotient (- scene-height (get-pipe-gap-y (first pipes)) pipe-self-gap) 2))
